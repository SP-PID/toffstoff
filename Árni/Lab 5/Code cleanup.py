...
class MainMenu:
    ...
    def run_state(self):
        ...
        cumerr1 = 0
        while True:
            set_rpm = pot1.Read_custom(0,13000)
            
            rpms = drive.rpm()
            
            curr_time = time.monotonic()
            elp_time = curr_time - prev_time
            
            err1 = set_rpm - rpms[0]
            err2 = set_rpm - rpms[1]
            
            
            cumerr1 = cumerr1 + err1 * (elp_time)
            cumerr2 = cumerr2 + err2 * (elp_time)
            #print(cumerr1)

            error1 = p*err1 + i*cumerr1
            error2 = p*err2 + i*cumerr2

            throttle_1 = simpleio.map_range(error1,0,13000,0,255)
            throttle_2 = simpleio.map_range(error2,0,13000,0,255)

            
            drive.throttle(throttle_2,throttle_1,"FWD")
            prev_time = curr_time
            print((set_rpm, rpms[1], rpms[0]))
            if ESTOP.Read():
...
class Pot:
    ...
    def Read_255(self):
        return simpleio.map_range(pot1.Read(),0,65535,0,255)

    def Read_custom(self, min, max):
        return simpleio.map_range(pot1.Read(),0,65535,min,max)
