...
E_stop = Button(E_Stop)
Mode = Button(Mode_switch)
Select = Button(Select_sw)
...
def IR_test():
    ...
    while True:
        ...
        if E_stop.press(): # Emg stop to send to Start mode
            modes = 0
            return True
        if Mode.press():
            return False
...
def Color_match():
    ...
    while True:
        ...
        if E_stop.press(): # Emergency stop, to stop whatever is happening and send to Start
...
def start():
    ...
    while True:
        if Mode.press(): # way to increment the modes value
            modes += 1 # Makes the mode value increas by one
            while Mode.press(): # This is an easy way to make it increment once
...
def Wait():
    ...
    while True:
        if Mode.press(): # incrementing the modes value
            modes += 1
            return False
        if E_stop.press(): # Emg stop to go to Start mode
            modes = 0
            return True
...
def Tachometer():
    ...
    while True: # Runs for 5 secons
        ...
        increment_A = encA.count
        increment_B = encB.count          # increment reader
        ...
        if E_stop.press(): # EMG stop to send to Start
            modes = 0
            return True
        if Mode.press():
            return False
