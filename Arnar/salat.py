...
class MainMenu:
    ...
    def run_state(self):
        ...
        while True:
            ...
            if clear_val[3] > 1000:
                    disp.text_to_display_2("off course",last)
                    if last == "Blue":
                        drive.throttle(-40,40,"FWD")
                    if last == "RED":
                        drive.throttle(40,-40,"FWD")
            
            elif color[0] > color[1] and color[0] > color[2] and clear_val[3] < 1000:
                color = color_sense.rgb_values()
                drive.throttle(45,-10,"FWD")
                color = color_sense.rgb_values()
                last = "RED"
                clear_val = color_sense.raw_data()
                ...
                drive.throttle(45,45,"FWD")
                color = color_sense.rgb_values()
                clear_val = color_sense.raw_data()
                
            elif color[2] > color[0] and color[2] > color[1] and clear_val[3] < 1000:
                color = color_sense.rgb_values()
                drive.throttle(-10,45,"FWD")
                color = color_sense.rgb_values()
                last = "Blue"
                clear_val = color_sense.raw_data()
...
class Drive:
    ...
    def pid_drive(self):
        ...
        while True:
            ...
            if pid.SetPoint > feedback:
                feedback += l2
                pid.update(feedback)
                print(l1,l2)
                output = pid.output
                a1,l1,a2,l2 = self.velocity()
                self.throttle((output),240,"FWD")
            print(l1,l2)
            print("feedback",feedback,"Set.Point",pid.SetPoint)
            #print(l1, l2)
            self.throttle((output),240,"FWD")
...
class PID:
    ...
    def __init__(self, P=1, I=0.2, D=0.1, current_time=1):
