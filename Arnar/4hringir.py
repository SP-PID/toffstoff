...
class MainMenu:
    ...
    def run_state(self):
        while True:
            ...
            print(clear_val)
            #print(color)
            if color[0] > color[1] and color[0] > color[2] and clear_val[3] < 1500:
                color = color_sense.rgb_values()
                drive.throttle(35,-35,"FWD")
                color = color_sense.rgb_values()
                
                clear_val = color_sense.raw_data()
                #a1,l1,a2,l2 =drive.velocity()
                ...
                #a1,l1,a2,l2 =drive.velocity()
                #disp.text_to_display_4("Ang.Vel.L:{0:0.2f} rad/s".format(a1),"Lin.Vel.L:{0:0.2f} m/s".format(l1),"Ang.Vel.R:{0:0.2f} rad/s".format(a2),"Lin.Vel.R:{0:0.2f} m/s".format(l2))
            elif color[2] > color[0] and color[2] > color[1] and clear_val[3] < 1500:
                color = color_sense.rgb_values()
                drive.throttle(-35,35,"FWD")
                color = color_sense.rgb_values()
                clear_val = color_sense.raw_data()
