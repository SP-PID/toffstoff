...
class DC_thread():
    ...
    def run(self):
        ...
        motor_Ain2.duty_cycle = 0            
        time.sleep(3)
        gv.disttravel = abs(dc_encoders.i)
        dc_encoders.i = 0 
        ratio = gv.disttravel / 775
        print(gv.disttravel)
        while True:
            PWM_val = pid_regulator.regulate(gv.sp * ratio, gv.dc_enc_val)
            gv.PWM_val = PWM_val
            ...
            if PWM_val < 0 and e_stop_top.value == False:
                motor_Ain1.duty_cycle = abs((PWM_val))*255
                motor_Ain2.duty_cycle = 0
            
            elif PWM_val > 0 and e_stop_bot.value == False:
                motor_Ain1.duty_cycle = 0
...
def Big_Plot(i, y1, y2):
    ...
    y = gv.dc_enc_val * (gv.disttravel/775)
    y2.append(y)
    y2 = y2[-x_len:]
